...
function ResultBySearch({ navigation, route }) {
  ...
  const homeAds = [
    {
      id: "0",
      coordinates: { latitude: 31.2587, longitude: 32.2988 },
      price: "200",
    ...
    {
      id: "1",
      coordinates: { latitude: 33.2587, longitude: 32.2988 },
      price: "240",
      image: require("../../assets/images/provider.png"),
      name: "مطعمll",
      location: "السعوديةالرياض",
  ...
  const fetchData = async () => {
    ...
    if (status !== "granted") {
      ...
      if (route.params && route.params.latitude) {
        ...
        ).then(() => setScreenLoader(false));
      } else {
        userLocation = { latitude, longitude, latitudeDelta, longitudeDelta };
        dispatch(
          getProviders(lang, null, false, latitude, longitude, homeSearch)
        ).then(() => setScreenLoader(false));
      }
      setInitMap(false);
    ...
    getCity +=
      "&key=AIzaSyCJTSwkdcdRpIXp2yG7DfSRKFWxKhQdYhQ&language=ar&sensor=true";
  ...
  function Item({ name, image, location, rate, type, id, index }) {
    return (
      <TouchableOpacity
        onPress={() => navigation.navigate("categoryDetails", { id, type })}
        style={[
          styles.borderGray,
          ...
          {active === 0 ? (
            ...
            <View
              style={[
                ...
                styles.bg_White,
                styles.Width_100,
                { overflow: "hidden", height: height - 200 },
              ]}
            >
              <View
                style={[
                ...
                >
                  ...
                  {providers
                    ? providers.map((marker, i) => (
                        ...
                        >
                          <Image
                            source={require("../../assets/images/red_location.png")}
                            resizeMode={"contain"}
                            style={[styles.icon20]}
                          />
              ...
              {showAd && popInfo ? (
                <View
                  style={[
                    styles.paddingHorizontal_15,
                    styles.Width_100,
                    { position: "absolute", bottom: 45, zIndex: 1 },
                  ]}
                >
                  <TouchableOpacity
